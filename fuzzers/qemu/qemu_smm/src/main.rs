...
fn main() {
    ...
    let ovmf_code_copy = project_path.join(fuzz_tag.clone()).join("OVMF_CODE.fd");
    let ovmf_var_copy = project_path.join(fuzz_tag.clone()).join("OVMF_VARS.fd");
...
fn fuzz((seed_path,corpus_path, crash_path, snapshot_path) : (&PathBuf, &PathBuf, &PathBuf, &PathBuf), fuzz_time : Option<Duration>, use_snapshot : bool, save_tmp_snapshot : bool) {
    ...
    if use_snapshot {
        ...
        let wrmsr_id = emulator.modules_mut().wrmsr(Hook::Closure(Box::new(move |modules, _state: Option<&mut _>, in_ecx: u32, in_eax: *mut u32, in_edx: *mut u32, handled : *mut bool| {
            wrmsr_smm_fuzz_phase(in_ecx, in_eax, in_edx, handled);
        })));
    ...
    let wrmsr_id = emulator.modules_mut().wrmsr(Hook::Closure(Box::new(move |modules, _state: Option<&mut _>, in_ecx: u32, in_eax: *mut u32, in_edx: *mut u32, handled : *mut bool| {
        wrmsr_init_fuzz_phase(in_ecx, in_eax, in_edx, handled);
    })));
    let mut memrw_id = emulator.modules_mut().memrw(Hook::Closure(Box::new(move |modules, _state: Option<&mut _>, pc : GuestAddr, addr : GuestAddr, size : u64, out_addr : *mut GuestAddr, rw : u32 , value : u128| {
        let fuzz_input = unsafe {&mut (*GLOB_INPUT) };
    ...
    loop {
        match snapshot {
            ...
            SnapshotKind::StartOfSmmInitSnap(_) => {
                ...
                (snapshot, skiped) = init_phase_fuzz(seed_dirs, corpus_dir.clone(), crash_dir, &mut emulator); 
                if skiped {
                    fs::remove_file(&snapshot_bin);
                } else {
