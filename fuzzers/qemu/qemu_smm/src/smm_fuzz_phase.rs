...
pub fn smm_phase_fuzz(seed_dirs : [PathBuf;1], corpus_dir : PathBuf, objective_dir : PathBuf, emulator: &mut Emulator<NopCommandManager, NopEmulatorExitHandler, (EdgeCoverageModule, (CmpLogModule, ())), StdState<MultipartInput<BytesInput>, CachedOnDiskCorpus<MultipartInput<BytesInput>>, libafl_bolts::prelude::RomuDuoJrRand, CachedOnDiskCorpus<MultipartInput<BytesInput>>>>)
{
...
pub fn smm_phase_run_one(tmp_dir : PathBuf, input : String, emulator: &mut Emulator<NopCommandManager, NopEmulatorExitHandler, (EdgeCoverageModule, (CmpLogModule, ())), StdState<MultipartInput<BytesInput>, CachedOnDiskCorpus<MultipartInput<BytesInput>>, libafl_bolts::prelude::RomuDuoJrRand, CachedOnDiskCorpus<MultipartInput<BytesInput>>>>)
{
    let qemu = emulator.qemu();
    let cpu: CPU = qemu.first_cpu().unwrap();
    ...
    test_cases.push(PathBuf::from_str(input.as_str()).unwrap());
    state.load_initial_inputs_by_filenames(&mut fuzzer, &mut executor, &mut mgr, test_cases.as_slice());

}


pub fn smm_phase_run_corpus(tmp_dir : PathBuf, corpus : PathBuf, emulator: &mut Emulator<NopCommandManager, NopEmulatorExitHandler, (EdgeCoverageModule, (CmpLogModule, ())), StdState<MultipartInput<BytesInput>, CachedOnDiskCorpus<MultipartInput<BytesInput>>, libafl_bolts::prelude::RomuDuoJrRand, CachedOnDiskCorpus<MultipartInput<BytesInput>>>>)
{
    let qemu = emulator.qemu();
    let cpu: CPU = qemu.first_cpu().unwrap();
    ...
    let mut corpus_dir = Vec::new();
    corpus_dir.push(corpus);
    state.load_initial_inputs(&mut fuzzer, &mut executor, &mut mgr, &corpus_dir.as_slice());
